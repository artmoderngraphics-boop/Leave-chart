window.addEventListener("unhandledrejection",function(e){try{var d=document.getElementById("bootErr");if(d){d.style.display="block";d.textContent="\u26a0\ufe0f Async error: "+((e&&e.reason&&((e.reason&&e.reason.message)||e.reason))||"unknown")}}catch(_){}});
...
function renderAll(){var _m="";try{renderStats();_m+="S"}catch(e){_m+="s!"}try{renderDashboard();_m+="D"}catch(e){_m+="d!"}try{renderCalendar();_m+="C"}catch(e){_m+="c!"}try{renderChart();_m+="Y"}catch(e){_m+="y!"}try{renderAll();_m+="A"}catch(e){_m+="a!"}try{renderTaken();_m+="T"}catch(e){_m+="t!"}try{renderUpcoming();_m+="U"}catch(e){_m+="u!"}try{renderTeam();_m+="E"}catch(e){_m+="e!"}try{renderSched();_m+="H"}catch(e){_m+="h!"}try{syncAdminUI();_m+="G"}catch(e){_m+="g!"}try{updateSyncPill();_m+="P"}catch(e){_m+="p!"}try{var _el=document.getElementById("lcBuild");if(_el&&_el.textContent.length<90)_el.textContent+=" ["+_m+"]"}catch(_){}}
...
async function loadFromJson(silent,force){
  try{
    ...
    const r=await fetch("data2.json?v="+Date.now(),{cache:"no-store"});
    if(!r.ok)throw 0;
...
function bootSync(){loadFromJson(true,staff.length===0&&leaves.length===0).then(()=>{try{fbInit()}catch(e){}try{gInit()}catch(e){}updateSyncPill()})}
function fbPush(){
...
try{applyLang()}catch(e){console.error("boot",e);dbgErr("boot: "+(e&&e.message||e));try{renderAll()}catch(_){}}try{syncAdminUI()}catch(e){console.error("boot",e);dbgErr("boot-admin: "+(e&&e.message||e))}try{bootSync()}catch(e){console.error("boot",e);dbgErr("boot-sync: "+(e&&e.message||e))};try{document.getElementById("lcBuild").textContent+=" \u2713"}catch(_){}
